...
const setListAssurance = (data) => ({
    type: SET_LIST_ASSURANCE,
    listAssurances: data,
    loading: false
})

const setCurrentAssurance = (data) => ({
    type: SET_CURRENT_ASSURANCE,
    currentAssurance: data,
    loading: false
})
...
const assuranceReducer = (state = initState, action) => {
    switch (action.type) {
        case SET_LIST_ASSURANCE:
            return { ...state, listAssurances: action.listAssurances, loading: action.loading }
        case SET_CURRENT_ASSURANCE:
            return { ...state, currentAssurance: action.currentAssurance, loading: action.loading }
        case SET_LOADING:
...
export function thunkListAssurances() {
    return async (dispatch) => {
        ...
        Axios({
            ...
            console.log(rows)
            dispatch(setListAssurance(rows))
        })
    }
}
...
export function thunkSearchAssurance(info) {
    return async (dispatch) => {
        Axios({
            method: "GET",
            url: `${header.url}/gap/search/assurance/${info}`,
        }).then(({ data: { rows } }) => {
            dispatch(setListAssurance(rows))
        })
    }
...
export function thunkDetailsAssurance(idAssurance) {
    return async (dispatch) => {
        Axios({
            method: "GET",
            url: `${header.url}/gap/details/assurance/${idAssurance}`,
        }).then(({ data: { rows } }) => {
